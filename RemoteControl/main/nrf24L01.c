...
static esp_err_t nrf_read_buf(uint8_t reg, uint8_t *buf, size_t len)
{
    ...
    tx[0] = NRF_R_REGISTER | (reg & 0x1F);
    memset(&tx[1], NRF_NOP, len);
...
static esp_err_t nrf_write_buf(uint8_t reg, const uint8_t *buf, size_t len)
{
    ...
    uint8_t tx[33] = {0};   // 1字节命令 + 最多32字节数据
    tx[0] = NRF_W_REGISTER | (reg & 0x1F);
    memcpy(&tx[1], buf, len);
...
esp_err_t nrf24_init(const nrf24_config_t *cfg)
{
    ...
    ESP_RETURN_ON_ERROR(nrf_write_reg(REG_EN_AA, 0x00), TAG, "en_aa failed");          // 只开 pipe0 自动应答
